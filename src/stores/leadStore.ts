...
export interface LeadFormData {
  custom_currency: string;
  currency: string; // Keep this field if needed elsewhere
  custom_fixed_charges: number;
...
const initialFormData: LeadFormData = {
  ...
  custom_currency: 'INR',
  currency: 'INR', // Add currency with default value
};
...
export const useLeadStore = create<LeadStore>()(
  devtools(
    (set, get) => ({
        ...
        set(
          (state) => ({
            leads: state.leads.map((lead) =>
              lead.id === id || lead.name === id ? { ...lead, ...updatedLead } : lead
            ),
          }),
          false,
          'updateLead'
      ...
      updateFormField: (field, value) =>
        set(
          (state) => {
            ...
            if (
              field === 'custom_fee' ||
              field === 'custom_estimated_hiring_' ||
              field === 'custom_average_salary' ||
              field === 'custom_fixed_charges' // Include custom_fixed_charges
            ) {
              const fee = field === 'custom_fee' ? value : newFormData.custom_fee;
              const fixedCharges =
                field === 'custom_fixed_charges' ? value : newFormData.custom_fixed_charges;
              const hiring =
                field === 'custom_estimated_hiring_'
                  ? value
                  : newFormData.custom_estimated_hiring_;
              const salary =
                field === 'custom_average_salary'
                  ? value
                  : newFormData.custom_average_salary;

              // Calculate deal value based on fee type
              const dealValue =
                fixedCharges && fixedCharges > 0
                  ? (fixedCharges * hiring) / 100000 // Fixed fee in lakhs
                  : fee && hiring && salary
                  ? (fee * hiring * salary) / 100 // Percentage-based in lakhs
                  : 0;
      ...
      calculateDealValue: () => {
        const { formData } = get();
        const { custom_fee, custom_estimated_hiring_, custom_average_salary, custom_fixed_charges } =
          formData;

        if (custom_fixed_charges && custom_fixed_charges > 0) {
          return (custom_fixed_charges * (custom_estimated_hiring_ || 0)) / 100000;
        }
      ...
      buildLeadPayload: () => {
        ...
        return {
          ...
          country: formData.company.country, // Use company country
          ...
          custom_fixed_charges: formData.custom_fixed_charges,
          custom_deal_value: formData.custom_deal_value,
          custom_expected_close_date: formData.custom_expected_close_date,
          custom_currency: formData.custom_currency, // Include custom_currency
        };
      },
    }),
    { name: 'lead-store' }
