...
const FRAPPE_BASE_URL = process.env.NEXT_PUBLIC_FRAPPE_BASE_URL || 'https://recruiter.gennextit.com';
...
export const frappeAPI = {
  ...
  getUserDetails: async (username: string) => {
    try {
      const response = await frappeClient.get(`/api/resource/User/${encodeURIComponent(username)}`);
      return {
        success: true,
  ...
  checkFirstLogin: async (username: string) => {
    try {
      ...
      const response = await frappeClient.get(
        `/api/resource/User/${encodeURIComponent(username)}?fields=["last_login","creation"]`
      );
  ...
  resetFirstTimePassword: async (username: string, newPassword: string) => {
    try {
      ...
      const response = await frappeClient.post('/api/method/frappe.core.doctype.user.user.update_password', {
