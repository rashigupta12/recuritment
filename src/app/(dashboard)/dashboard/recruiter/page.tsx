...
const processedTrendsData = useMemo((): TrendData[] => {
  ...
  // For month period, use periods directly from API response
  if (periodInfo?.type === "month") {
    return existingTrends.map((trend: TrendData) => ({
      period: trend.period,
      totalCVUploaded: trend.totalCVUploaded,
      open: trend.open,
      tagged: trend.tagged,
      shortlisted: trend.shortlisted,
      assessment: trend.assessment,
      interview: trend.interview,
      interviewReject: trend.interviewReject,
      offered: trend.offered,
      offerDrop: trend.offerDrop,
      joined: trend.joined,
    }));
  }

  // Existing logic for week and quarter periods
